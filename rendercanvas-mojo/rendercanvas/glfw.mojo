...
struct GLFWLib(Movable):
    ...
    def create_window(
        ...
        width: Int32,
        height: Int32,
        title: OpaquePointer[MutUntrackedOrigin],
        monitor: OpaquePointer[MutUntrackedOrigin] = null_opaque(),
        share: OpaquePointer[MutUntrackedOrigin] = null_opaque(),
    ) -> OpaquePointer[MutUntrackedOrigin]:
        """Create a GLFW window; returns GLFWwindow* (or NULL on failure)."""
        return self._lib.call["glfwCreateWindow", OpaquePointer[MutUntrackedOrigin]](
            width, height, title, monitor, share
        )

    def destroy_window(self, window: OpaquePointer[MutUntrackedOrigin]):
        self._lib.call["glfwDestroyWindow"](window)

    # --- Event loop ---------------------------------------------------

    def window_should_close(self, window: OpaquePointer[MutUntrackedOrigin]) -> Int32:
    ...
    def get_wayland_display(self) -> OpaquePointer[MutUntrackedOrigin]:
        """Returns wl_display* for the Wayland display connection.

        Returns NULL if GLFW is not running on Wayland.
        """
        return self._lib.call["glfwGetWaylandDisplay", OpaquePointer[MutUntrackedOrigin]]()

    def get_wayland_window(self, window: OpaquePointer[MutUntrackedOrigin]) -> OpaquePointer[MutUntrackedOrigin]:
        """Returns wl_surface* for the given GLFW window on Wayland."""
        return self._lib.call["glfwGetWaylandWindow", OpaquePointer[MutUntrackedOrigin]](window)

    # --- X11 native pointers (XWayland / bare X11 fallback) ----------

    def get_x11_display(self) -> OpaquePointer[MutUntrackedOrigin]:
        """Returns X11 Display* pointer."""
        return self._lib.call["glfwGetX11Display", OpaquePointer[MutUntrackedOrigin]]()

    def get_x11_window(self, window: OpaquePointer[MutUntrackedOrigin]) -> UInt64:
        """Returns X11 Window (unsigned long) for the given GLFW window."""
        return self._lib.call["glfwGetX11Window", UInt64](window)

    # --- Keyboard polling -----------------------------------------------

    def get_key(self, window: OpaquePointer[MutUntrackedOrigin], key: Int32) -> Int32:
        """Query key state: returns GLFW_PRESS or GLFW_RELEASE."""
        return self._lib.call["glfwGetKey", Int32](window, key)

    # --- Mouse polling --------------------------------------------------

    def get_mouse_button(self, window: OpaquePointer[MutUntrackedOrigin], button: Int32) -> Int32:
    ...
    def set_cursor_pos(self, window: OpaquePointer[MutUntrackedOrigin], xpos: Float64, ypos: Float64):
        """Set cursor position in window coordinates."""
        self._lib.call["glfwSetCursorPos"](window, xpos, ypos)

    # --- Input mode -----------------------------------------------------

    def set_input_mode(self, window: OpaquePointer[MutUntrackedOrigin], mode: Int32, value: Int32):
        """Set input mode (e.g. GLFW_CURSOR → GLFW_CURSOR_DISABLED)."""
        self._lib.call["glfwSetInputMode"](window, mode, value)

    def get_input_mode(self, window: OpaquePointer[MutUntrackedOrigin], mode: Int32) -> Int32:
        """Get current input mode value."""
        return self._lib.call["glfwGetInputMode", Int32](window, mode)

    # --- C callback bridge (event queue) --------------------------------

    def install_input_callbacks(self, window: OpaquePointer[MutUntrackedOrigin]):
    ...
    def remove_input_callbacks(self, window: OpaquePointer[MutUntrackedOrigin]):
