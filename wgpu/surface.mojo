...
struct SurfaceFrame(TrivialRegisterPassable):
    ...
    status values:
      1 = SuccessOptimal   — render normally
      2 = SuccessSuboptimal — render; resize recommended
      other               — texture is null; skip frame
    """
...
struct Surface(Movable):
    ...
    def __init__(out self, var lib: WGPULib, handle: WGPUSurfaceHandle):
        self._lib    = lib^
        self._handle = handle
        self._format = 0
        self._width  = 0
    ...
    def handle(self) -> WGPUSurfaceHandle:
        return self._handle
    ...
    # ------------------------------------------------------------------
    # Factory — platform-specific surface creation
    # ------------------------------------------------------------------
